...
class AlliesBoard extends React.Component {
  render() {
    const handleTouchStartPad = (event, { coordinate }) => {
      this.props.dispatch(moveCursor(CURSOR_BELONGING_TYPES.ALLIES_BOARD, coordinate));
    };
...
AlliesBoard = connect(state => {
  const cursorCoordinate =
    state.cursor.cursorBelongingType === CURSOR_BELONGING_TYPES.ALLIES_BOARD ?  state.cursor.coordinate : null;
